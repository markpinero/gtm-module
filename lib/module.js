...
module.exports = async function gtmModule (_options) {
  ...
  const doNotTrackEnabled = !!options.respectDoNotTrack
  const doNotTrackScript = doNotTrackEnabled ? 'if(w.doNotTrack||w[x][i])return;' : ''
  ...
  script = `${doNotTrackEnabled && dnt};(function(w,d,s,l,x,y){${script}})(window,document,'script','${options.layer}','_gtm_ids','_gtm_inject')`
